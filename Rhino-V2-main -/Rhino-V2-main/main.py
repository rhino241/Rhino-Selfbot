...
| |\ \   | | |)|| || | |)|\ `-' /  
|_| \)\  /(  (_)`-'/(  (_) )---'   
(__)(__)      (__)    (_)  v{__version__}{Style.RESET_ALL}""")
...
try:
   set_terminal_title("| Rhino-V2 Selfbot |")
...
#try:
#    if float(__version__) > float(check_latest_version('Rhino', 'Rhino-V2').strip('v')):
#        log.warning(f"{langs.unstable_version[config_selfbot.lang]} https://github.com/Rhino/Rhino-V2/releases/tag/{check_latest_version('Rhino', 'Rhino-V2')}")
#        check_loop = False
#except Exception:
#    pass
...
if not discord.__title__ == "discord.py-self":
    log.critical(f"{langs.error_discord_version[config_selfbot.lang]} https://github.com/Rhino/{check_latest_version('Rhino', 'Rhino-V2')}")
    exit()
...
#        run_in_background()
#    except Exception as e:
#        log.warning(f"Error while trying to check the last Rhino version: {e}")
#
...
async def on_ready():
    ...
    if rpc.read_variable_json("create_panel"):
        with open('Rhino_icon.png', 'rb') as image:
            Rhino_icon = image.read()
        panel = await bot.create_group()
        await asyncio.sleep(0.7)
        await panel.edit(name="Rhino Panel", icon=Rhino_icon)
        ...
        log.alert("RhinoPanel successfully created (check DMs!).")
